...
* `type` - character string specifying the type of the input data. Two types implemented are "RDF" - the R.ROSETTA output (see  [R.Rosetta format](#RDF)), "L" - "Line by line" file format (see ['line by line' format](#Lbl)) The default is "RDF"
* `NodeColorType` - character string specifying the color of nodes: 
    * `"DL"` - feature discretization levels, option is available for data discretized into three levels: 1 (yellow), 2 (grey) and 3 (blue). In the case of the gene expression, data dicretization levels correpond to: 1 - undeexpressed gene, 2 - no change gene expression and 3 - overexpressed gene.
    * `"A"` - color of nodes defined by the mean accuracy value for the node: dark orange - high accuracy value, ligth orange - low accuracy value. 
...
* `CustObjectNodes` - a list that contains customised VisuNet output for nodes. The list needs to contain two variables:
...
* `CustObjectEdges` - a list that contains customised VisuNet output for edges. The list needs to contain two variables:
...
The VisuNet output is a set of lists corresponds to one decision variable plus one extra list for combined decision 'all'.
...
* `color.background`  - the node color, see **color nodes section** for details
* `value` - the node size
...
* `connNorm`  - the connection variable normalized addcorid to the maximum connection variable in the rule network
* `label2` - the edge id
