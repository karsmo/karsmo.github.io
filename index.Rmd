...
This example uses gene expression data for autistic and non-autistic young males (@RN1). The rule-based classifier was created using [R.ROSETTA](https://github.com/komorowskilab/R.ROSETTA.git) [see @RN2].
...
```{r, echo = FALSE, warning=FALSE, fig.cap='The sample rule network for the autistic and non-autistic young males classifier from VisuNet (constructed for min decision coverage=27% and min accuracy=88%)'}
